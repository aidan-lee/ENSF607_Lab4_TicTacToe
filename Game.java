...
public class Game implements Constants {
	...
	static public Player  create_player(String name, char mark, Board board,
		...
		while (!validResponse) {
			player_type = getPlayerType(stdin, NUMBER_OF_TYPES);
			if (player_type == -1) {
	...
	static private int getPlayerType(BufferedReader stdin, int typeNum) {
		try {
			String input= stdin.readLine();
			int player_type = Integer.parseInt(input);
			while (player_type < 1 || player_type > typeNum) {
				System.out.print( "Please try again.\n");
				System.out.print ( "Enter a number in the range 1-" + typeNum + ": ");
